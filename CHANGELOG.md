...
- **Startup warns when DoS-relevant limits are left unbounded** — `--max-connections`, `--max-connections-per-ip`, `--max-waiters`, and (when the HTTP API is enabled) `--http-max-sessions` / `--http-max-connections-per-ip` / `--http-rate-limit-per-ip` — unless the server is bound only to loopback.
